...
def message_info_p():
    mb0 = messagebox.showinfo(
        "showinfo", "p is prime and must be different from q ")


def message_info_q():
    mb0 = messagebox.showinfo(
        "showinfo", "q is prime and must be different from p ")


def message_info_e():
    mb0 = messagebox.showinfo(
        "showinfo", "e is a prime number and must be in the range from 1 to phi(n) ")


def message_info_input():
    mb0 = messagebox.showinfo(
        "showinfo", "1.Character: enter as letters without numbers. \n2.Number: Must enter number and must be in the correct format 1 # 2 # 3... with a space between the number and '#' ")
...
def calc_de_num(text_input, e, p, q, frame1, frame2):
    ...
    if (any(ch.isdigit() for ch in mystring) and '#' and ' ') or len(mystring) >= 0:
        ...
        if n == True:
            ...
            delete1(frame2)
            messagebox.showerror(
                'message', 'The decrypted message could not be decoded into a text message!')
    else:
        delete(frame1)
        delete1(frame2)
        messagebox.showerror('message', 'Name can\'t have characters')
...
def main():
    ...
    q_buttonInfo = tk.Button(frame, text="?", width=8, command=message_info_q)
    ...
    e_buttonInfo = tk.Button(frame, text="?", width=8, command=message_info_e)
    ...
    text_buttonInfo = tk.Button(
        frame1, text="?", width=4, command=message_info_input)
